...
const Hero = () => {
  ...
  const handleMiniVdClick = () => {
    ...
    if (nextVideoRef.current) {
      ...
      nextVideoRef.current.play().catch((error) => console.error("Playback error:", error));
    }
  };

  // Ensure isLoading updates properly
  useEffect(() => {
  ...
  // GSAP Animation for Video Expansion
  useGSAP(() => {
    if (hasClicked) {
  ...
  // GSAP Scroll Animation for Clip Path
  useGSAP(() => {
  ...
  return (
    <section className="relative h-dvh w-screen overflow-x-hidden">
      ...
      <div id="video-frame" className="relative z-10 h-dvh w-screen overflow-hidden rounded-lg bg-blue-100">
        <div>
          {/* Mini Video Click Box */}
          <div id="home" className="absolute-center absolute z-50 size-64 cursor-pointer overflow-hidden rounded-lg">
            <div
          ...
          {/* Hidden Video (Expands when clicked) */}
          <video
            ref={nextVideoRef}
